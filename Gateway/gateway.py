...
class Gateway:
    def __init__(self, host, port_temp, port_hum):
        ...
        self.temperature_clock_thread = self.create_temperature_clock(3)
        self.humidity_clock_thread = self.create_humidity_clock(7)
    ...
    def create_humidity_clock(self, time: int):
        clock_thread = threading.Thread(target=self.start_humidity_clock, args=(time))
        return clock_thread
    
    def create_temperature_clock(self, time: int):
        clock_thread = threading.Thread(target=self.start_temperature_clock, args=(time))
        return clock_thread

    def listen_humidity(self):
        while True:
            message = self.humidity_socket.recv(1024).decode('utf-8')
    ...
    def listen_temperature(self):
        while True:
            connection, _ = self.temperature_socket.accept()
    ...
    def start_temperature_clock(self, time: int):
    ...
    def start_humidity_clock(self, time: int):
        start_time = time_module.time()
        while True:
            if self.humdity_alive is False:
                current_time = time_module.time()
                passed_time = start_time - current_time
                if passed_time >= time:
                    break
